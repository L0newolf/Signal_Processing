...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
extern  u8 EthFrame[ENET_MAX_PACKET_SIZE], buffer1[ENET_MAX_PACKET_SIZE], 
           buffer2[ENET_MAX_PACKET_SIZE], buffer3[ENET_MAX_PACKET_SIZE];
extern u16 TabSize[ENET_TXBUFNB];
extern u16 size1, size2, size3;
...
void ENET_IRQHandler(void)
{
  ...
  ENET_ClearIT(ENET_IT_RX_CURR_DONE); 
    
  switch (i)
  { 
  case 1:
    size1 = ENET_DMACpyRxPkt(buffer1);
    i++;
    break;
     
  case 2:
    size2 = ENET_DMACpyRxPkt(buffer2);
    i++;
    break;
      
  case 3:
    size3 = ENET_DMACpyRxPkt(buffer3);
    i++;
    break;
...
{
  /* Clear the pending bit */
  DMA_ClearIT(Channel1, DMA_TCC);
  
  /* Update the current ENET DMA descriptor */
  ENET_UpdateRxDscr();
