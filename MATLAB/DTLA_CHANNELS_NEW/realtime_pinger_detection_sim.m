...
[data,Fs] = readDTLA('/Users/anshu/Desktop/031016_164127.dat',1:24,tStart,tDur);
...
for i=1:numLoops
    ...
    for j = 1:numChannels
        ...
        [maxPow(i,j),sigFreq(i,j),~]  = findSigFreq( curSig(:,j),Fs,nFFT,samplesPerBlock,overlap );
        
    end
   
    normVal = max(maxPow(i,:));
    maxPow(i,:) = maxPow(i,:)/normVal;
    figure(1);
    plot(maxPow(i,:));
    pause(2);
...
%     angles = deg2rad(-90:3:90);
%     
%     startPtr = 1;
%     
%     bfoSub_real_new = zeros(samplesPerBlock,length(angles),numChannels);
%     bfoSub_imag_new = zeros(samplesPerBlock,length(angles),numChannels);
%     
%     for q=1:numChannels
%         [bfoSub_real_new(:,:,q),bfoSub_imag_new(:,:,q)] = beamFormer(curSig(:,q),Fs,freqBFO,q,angles,samplesPerBlock);
%     end
% 
...
%     end
%     
%     maxTime = durPerBlock*(i-1)+(timeIdx/Fs);
%     
...
%     
%     figure(1);
%     imagesc((1:size(bfoSub,1))/Fs,rad2deg(angles),bfoSub');
%     colorbar
%     clim = get(gca,'CLim');
