...
/*------------------- start of genFilt.h ---------------------------------------*/
 ...
 void BasicFIR(double *FirCoeff, int NumTaps, TBasicPassType PassType, float OmegaC, float BW, TWindowType WindowType, float WinBeta);
 void WindowData(double *Data, int N, TWindowType WindowType, float Alpha, float Beta, bool UnityGain);
...
float OmegaC  0.0 < OmegaC < 1.0   The filters corner freq, or center freq if BPF or NOTCH
float BW      0.0 < BW < 1.0       The filters band width if BPF or NOTCH
...
float Beta;  0 <= Beta <= 10.0     Beta is used with the Kaiser, Sinc, and Sine windows only.
...
float dBAtten, Beta, Beta1=0.0, Beta2=0.0;
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    TAPS = mxGetScalar(prhs[1]);
    omega = mxGetScalar(prhs[2]);
    BW = mxGetScalar(prhs[3]);
    
    /* create the output matrix */
    plhs[0] = mxCreateDoubleMatrix(1,TAPS,mxREAL);

    /* get a pointer to the real data in the output matrix */
    firCoeff = mxGetPr(plhs[0]);
    
    BasicFIR(firCoeff,TAPS,BPF,omega, BW, wtKAISER_BESSEL, beta);
}
