...
u8 buffer1[ENET_MAX_PACKET_SIZE], buffer2[ENET_MAX_PACKET_SIZE], buffer3[ENET_MAX_PACKET_SIZE];
...
void main()
{
  ...
  for(i=0;i<ENET_MAX_PACKET_SIZE;i++)
  {
    buffer1[i] = 0;
    buffer2[i] = 0;
    buffer3[i] = 0;
  ...
  /* Send Three packets */
  ENET_HandleTxPkt(EthFrame[0],Pkt1Length);
  ENET_HandleTxPkt(EthFrame[1],Pkt2Length);
  ENET_HandleTxPkt(EthFrame[2],Pkt3Length);
  
  /* Check the packets received */
