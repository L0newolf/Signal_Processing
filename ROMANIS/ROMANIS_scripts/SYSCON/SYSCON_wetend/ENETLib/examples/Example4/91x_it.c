...
{ 
  ...
  /* Clear the WIU line 25 pending bit */
  WIU_ClearITPendingBit(WIU_Line25);
  
  
  /* Clear the PHY interrupt flag and get the Auto-Negotiation configuration */
  regValue = ENET_PHYGetITSrc(PHY_ADDRESS);
  regValue = ENET_MIIReadReg(PHY_ADDRESS, PHY_XCIIS);
    if (regValue & 0x100) /*full duplex */
      {
        ENET_MAC->MCR |= ENET_MCR_FDM;   /* full duplex mode */
        ENET_MAC->MCR &= ~ENET_MCR_DRO;  /* enable frame reception during transmission */
        
        if(regValue & 0x200) /* the speed is 100M */
        {
          LCD_Clear();
          LCD_DisplayString(Line1, "100M Full", BlackText);  
        }
        
        else  /* the speed is 10M */
        {
          LCD_Clear();
          LCD_DisplayString(Line1, "10M Full", BlackText);  
        }
      }
    else /* half duplex */
      {
	...
	ENET_MAC->MCR |= ENET_MCR_DRO;  /* disable frame reception during transmission */
        ...
        {
          LCD_Clear();
          LCD_DisplayString(Line1, "100M Half", BlackText);  
        }
        
        else  /* the speed is 10M */
        {
          LCD_Clear();
          LCD_DisplayString(Line1, "10M Half", BlackText);  
        }
      }  
